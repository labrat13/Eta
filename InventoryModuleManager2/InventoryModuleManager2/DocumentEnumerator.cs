...
{
    /// <summary>
    /// NT-Итератор документов и изображений
    /// </summary>
    public class DocumentEnumerator: IEnumerator
    {
        ...
        /// </summary>
        private List<FileRecord> m_recordList;
        ...
        public DocumentEnumerator(String tablename, DbAdapter adapter)
        {
            m_tablename = tablename;
            m_adapter = adapter;
            m_recordList = new List<FileRecord>();
            //m_endId = adapter.GetTableMaxId(tablename);
            ////if (m_endId == -1) - TODO: no records in table
            //m_startId = adapter.GetTableMinId(tablename);
        ...
        public void Reset()
        {
            m_endId = m_adapter.getTableMaxId(m_tablename);
            //if (m_endId == -1) - TODO: no records in table
            m_startId = m_adapter.getTableMinId(m_tablename);
            //if(m_startId < 0) - no records in table
            m_currentId = 0;
            m_recordList.Clear();
            return;
        ...
        private void fillListWith16items()
        {
            ...
            {
                m_recordList.AddRange(m_adapter.getFileRecordsBetween(m_tablename, m_currentId, i+16));
